    ...
    module Deserializer
        ...
        include FHIR::Deserializer::Utilities
            ...
            def parse_xml_entry_PatientLinkComponent(entry) 
                ...
                self.parse_element_data(model, entry)
                set_model_data(model, 'other', FHIR::ResourceReference.parse_xml_entry(entry.at_xpath('./fhir:other')))
            ...
            def parse_xml_entry(entry) 
                ...
                set_model_data(model, 'careProvider', entry.xpath('./fhir:careProvider').map {|e| FHIR::ResourceReference.parse_xml_entry(e)})
                set_model_data(model, 'managingOrganization', FHIR::ResourceReference.parse_xml_entry(entry.at_xpath('./fhir:managingOrganization')))
