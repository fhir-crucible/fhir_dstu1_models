    ...
    module Deserializer
        ...
        include FHIR::Deserializer::Utilities
            def parse_xml_entry_ImagingStudySeriesInstanceComponent(entry) 
                ...
                set_model_data(model, 'attachment', FHIR::ResourceReference.parse_xml_entry(entry.at_xpath('./fhir:attachment')))
                model
            end
            ...
            def parse_xml_entry(entry) 
                ...
                set_model_data(model, 'uid', entry.at_xpath('./fhir:uid/@value').try(:value))
                set_model_data(model, 'accessionNo', FHIR::Identifier.parse_xml_entry(entry.at_xpath('./fhir:accessionNo')))
                set_model_data(model, 'identifier', entry.xpath('./fhir:identifier').map {|e| FHIR::Identifier.parse_xml_entry(e)})
                set_model_data(model, 'order', entry.xpath('./fhir:order').map {|e| FHIR::ResourceReference.parse_xml_entry(e)})
                set_model_data(model, 'modality', entry.xpath('./fhir:modality/@value').map {|e| e.value })
                set_model_data(model, 'referrer', FHIR::ResourceReference.parse_xml_entry(entry.at_xpath('./fhir:referrer')))
                set_model_data(model, 'availability', entry.at_xpath('./fhir:availability/@value').try(:value))
                set_model_data(model, 'url', entry.at_xpath('./fhir:url/@value').try(:value))
                ...
                set_model_data(model, 'interpreter', FHIR::ResourceReference.parse_xml_entry(entry.at_xpath('./fhir:interpreter')))
                set_model_data(model, 'description', entry.at_xpath('./fhir:description/@value').try(:value))
