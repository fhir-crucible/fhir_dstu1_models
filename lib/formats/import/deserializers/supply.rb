    ...
    module Deserializer
        ...
        include FHIR::Deserializer::Utilities
            def parse_xml_entry_SupplyDispenseComponent(entry) 
                ...
                set_model_data(model, 'suppliedItem', FHIR::ResourceReference.parse_xml_entry(entry.at_xpath('./fhir:suppliedItem')))
                set_model_data(model, 'supplier', FHIR::ResourceReference.parse_xml_entry(entry.at_xpath('./fhir:supplier')))
            ...
            def parse_xml_entry(entry) 
                ...
                set_model_data(model, 'identifier', FHIR::Identifier.parse_xml_entry(entry.at_xpath('./fhir:identifier')))
                set_model_data(model, 'status', entry.at_xpath('./fhir:status/@value').try(:value))
                set_model_data(model, 'orderedItem', FHIR::ResourceReference.parse_xml_entry(entry.at_xpath('./fhir:orderedItem')))
