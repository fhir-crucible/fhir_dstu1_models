    ...
    module Deserializer
        ...
        include FHIR::Deserializer::Utilities
            def parse_xml_entry_MessageHeaderResponseComponent(entry) 
                ...
                set_model_data(model, 'details', FHIR::ResourceReference.parse_xml_entry(entry.at_xpath('./fhir:details')))
                model
            end
            ...
            def parse_xml_entry_MessageSourceComponent(entry) 
                ...
                set_model_data(model, 'contact', FHIR::Contact.parse_xml_entry(entry.at_xpath('./fhir:contact')))
            ...
            def parse_xml_entry(entry) 
                ...
                set_model_data(model, 'enterer', FHIR::ResourceReference.parse_xml_entry(entry.at_xpath('./fhir:enterer')))
                set_model_data(model, 'author', FHIR::ResourceReference.parse_xml_entry(entry.at_xpath('./fhir:author')))
                set_model_data(model, 'receiver', FHIR::ResourceReference.parse_xml_entry(entry.at_xpath('./fhir:receiver')))
                set_model_data(model, 'responsible', FHIR::ResourceReference.parse_xml_entry(entry.at_xpath('./fhir:responsible')))
                set_model_data(model, 'reason', FHIR::CodeableConcept.parse_xml_entry(entry.at_xpath('./fhir:reason')))
                set_model_data(model, 'data', entry.xpath('./fhir:data').map {|e| FHIR::ResourceReference.parse_xml_entry(e)})
                model
            end
        end
    end
