    ...
    module Deserializer
        ...
        include FHIR::Deserializer::Utilities
            def parse_xml_entry_SpecimenSourceComponent(entry) 
                ...
                set_model_data(model, 'target', entry.xpath('./fhir:target').map {|e| FHIR::ResourceReference.parse_xml_entry(e)})
                model
            end
            ...
            def parse_xml_entry_SpecimenCollectionComponent(entry) 
                ...
                self.parse_element_data(model, entry)
                set_model_data(model, 'collector', FHIR::ResourceReference.parse_xml_entry(entry.at_xpath('./fhir:collector')))
            ...
            def parse_xml_entry_SpecimenTreatmentComponent(entry) 
                ...
                set_model_data(model, 'additive', entry.xpath('./fhir:additive').map {|e| FHIR::ResourceReference.parse_xml_entry(e)})
                model
            end
            ...
            def parse_xml_entry_SpecimenContainerComponent(entry) 
                ...
                set_model_data(model, 'additive', FHIR::ResourceReference.parse_xml_entry(entry.at_xpath('./fhir:additive')))
                model
            end
