module FHIR
    module Deserializer
        ...
        include FHIR::Deserializer::Utilities
            def parse_xml_entry_ValueSetDefineConceptComponent(entry) 
                return nil unless entry
                model = FHIR::ValueSet::ValueSetDefineConceptComponent.new
                ...
                set_model_data(model, 'concept', entry.xpath('./fhir:concept').map {|e| parse_xml_entry_ValueSetDefineConceptComponent(e)})
                model
            end
            ...
            def parse_xml_entry_ConceptSetComponent(entry) 
                ...
                set_model_data(model, 'system', entry.at_xpath('./fhir:system/@value').try(:value))
                set_model_data(model, 'versionNum', entry.at_xpath('./fhir:version/@value').try(:value))
                set_model_data(model, 'code', entry.xpath('./fhir:code/@value').map {|e| e.value })
