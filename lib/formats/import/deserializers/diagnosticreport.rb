    ...
    module Deserializer
        ...
        include FHIR::Deserializer::Utilities
            def parse_xml_entry_DiagnosticReportImageComponent(entry) 
                ...
                set_model_data(model, 'link', FHIR::ResourceReference.parse_xml_entry(entry.at_xpath('./fhir:link')))
                model
            end
            ...
            def parse_xml_entry(entry) 
                ...
                set_model_data(model, 'performer', FHIR::ResourceReference.parse_xml_entry(entry.at_xpath('./fhir:performer')))
                set_model_data(model, 'identifier', FHIR::Identifier.parse_xml_entry(entry.at_xpath('./fhir:identifier')))
                set_model_data(model, 'requestDetail', entry.xpath('./fhir:requestDetail').map {|e| FHIR::ResourceReference.parse_xml_entry(e)})
                ...
                set_model_data(model, 'specimen', entry.xpath('./fhir:specimen').map {|e| FHIR::ResourceReference.parse_xml_entry(e)})
                set_model_data(model, 'result', entry.xpath('./fhir:result').map {|e| FHIR::ResourceReference.parse_xml_entry(e)})
                set_model_data(model, 'imagingStudy', entry.xpath('./fhir:imagingStudy').map {|e| FHIR::ResourceReference.parse_xml_entry(e)})
