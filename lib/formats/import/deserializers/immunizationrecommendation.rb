    ...
    module Deserializer
        ...
        include FHIR::Deserializer::Utilities
            ...
            def parse_xml_entry_ImmunizationRecommendationRecommendationComponent(entry) 
                ...
                set_model_data(model, 'supportingImmunization', entry.xpath('./fhir:supportingImmunization').map {|e| FHIR::ResourceReference.parse_xml_entry(e)})
                set_model_data(model, 'supportingPatientInformation', entry.xpath('./fhir:supportingPatientInformation').map {|e| FHIR::ResourceReference.parse_xml_entry(e)})
                model
            end
